...
ggplot(Grub, aes(y = mcmc_subset_y[10,], x = grubsize, colour = as.factor(group))) + geom_point()+geom_line(color='black',data = predicted_df, aes(x=size, y=live1)) +
  geom_line(color='gray',data = predicted_df, aes(x=size, y=live2))
...
#Weibull Case
#if we have group 2 instead of group 1, we have have an increase of quantile survival time by ((exp(beta2)-1)*100)% (survival time changes by a factor of exp(beta2))
((exp(a1["beta2"])-1)*100)
